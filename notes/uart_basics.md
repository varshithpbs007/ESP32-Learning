...
* It's used and preferred for point-to-point communication of devices and has low speeds compared to I2C and SPI.
...
* Data frame format and transmission speeds ( Baud rate ) are configurable in UART

  ## Configuration of UART module before transmission
  * Both the transmit and receive devices must be configured with same baud rate in the BRR(Baud rate register)
  ...
  * Key Rule : For successful communication, both the Tx and Rx must be set to the same baud rate and the receiver can know how to slice the incoming signal into bits. If they dont match, the timing will be off and the data will appear as 'garbage' text or symbols.
  ...
  * since UART is asynchronous, it does not have a clock wire to tell the receiver when a bit starts and ends, so it uses the baud rate to determine the "width" of each bit in time
    1. Start bit : The default high line drops to low signalling start of a frame
    2. Data bits : Usually 8 bits of data are sent
    3. Parity bit (optional) : We can include one optional parity bit to detect any error
    4. Stop bit : The line returns to a high state signalling end of a data frame.
